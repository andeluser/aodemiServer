...
public class b56 implements CardAbility {
		...
		for (int i = 0; i < targetList.size(); i++) {
			...
			for (int j = 0; j < koList.size(); j++) {
				...
				for (int k= 0; k < list.size(); k++) {
					...
					fieldDto.setPermanent_frm(fieldDto.getPermanent_frm() + 3);
	...
	public HashMap<String, Object> action1(String battleID, String playerId, int fieldNumber) throws Exception {
		...
		ArrayList<Object> tmpTargetList = battleUtil.getRangeTargetList(controllDto, playerId, fieldNumber);

		ArrayList targetList = new ArrayList();

		for (int i = 0; i < tmpTargetList.size(); i++) {
			int target = (int)tmpTargetList.get(i);
			BattleFieldDTO list = fieldDao.getAllValue(battleID, enemyPlayerId, target);

			int frm =list.getPermanent_frm() + list.getTurn_frm() + list.getCur_frm();
			if (list.getCard_id() != null && !"".equals(list.getCard_id()) && list.getClose() == 0 && frm >= 8) {
				targetList.add(list.getField_no());
			}
		}
