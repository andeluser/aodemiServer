...
//吸いとる者
public class b74 implements CardAbility {
	...
	public HashMap<String, Object> auto(String battleID, String playerId, int fieldNumber) throws Exception {
		...
		for (int i = 0; i < result.size(); i++) {
			...
			int attack = (myDto.getPermanent_hp() + myDto.getTurn_hp() + myDto.getBase_hp()) / 2;
			attack = attack - def;
		...
		myDto.setPermanent_hp(myDto.getPermanent_hp() + 10);
		myDto.setCur_hp(myDto.getCur_hp() + 10);
		myDto.setAction(1);
		fieldDao.update(myDto);

		detailMap2.put("playerId", playerId);
		detailMap2.put("fieldNumber", fieldNumber);
		detailMap2.put("hp", myDto.getCur_hp());
		detailMap2.put("upHP", myDto.getPermanent_hp());
		detailMap2.put("remove", "actionEnd");
		list.add(detailMap2);
