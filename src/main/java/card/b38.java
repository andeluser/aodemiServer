...
//蛇目の参謀
public class b38 implements CardAbility {
		...
		for (int i = 0; i < targetList.size(); i++) {
			...
			for (int j = 0; j < koList.size(); j++) {
				...
				for (int k= 0; k < list.size(); k++) {
					...
					int agi = 1 - fieldDto.getCur_speed();
					fieldDto.setPermanent_speed(agi);

					int Perhp = 30 - fieldDto.getBase_hp();
	...
	public HashMap<String, Object> close(String battleID, String playerId, int fieldNumber) throws Exception {
		...
		for (int i = 0; i < enemyFieldDtoList.size(); i++) {
			...
			if (list.getCard_id() != null && !"".equals(list.getCard_id()) && list.getClose() == 0 && list.getCur_hp() == 30) {
