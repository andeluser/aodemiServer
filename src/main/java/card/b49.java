...
//冷気の波動使い
public class b49 implements CardAbility {
	...
	public HashMap<String, Object> open(String battleID, String playerId) throws Exception {
		...
		//対象を計算する,未アクションレベル3以下
		ArrayList<Object> targetList = new ArrayList<Object>();

		int level = 0;
		for (int i = 0; i < fieldDtoList.size(); i++) {
			BattleFieldDTO list = fieldDtoList.get(i);
			level =  list.getCur_level() + list.getTurn_level() + list.getPermanent_level();
			if (list.getCard_id() != null && !"".equals(list.getCard_id()) && list.getClose() == 0 && list.getAction() == 0 && level <= 3) {
