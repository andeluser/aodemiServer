...
public class m28 implements CardAbility {
	...
	public HashMap<String, Object> open(String battleID, String playerId) throws Exception {
		...
		for (int i = 0; i < enemyFieldDtoList.size(); i++) {
			...
			if (list.getCard_id() != null && !"".equals(list.getCard_id()) && list.getClose() == 0 && level <= 1) {
	...
	public HashMap<String, Object> action1(String battleID, String playerId, int fieldNumber) throws Exception {
		...
		for (int i = 0; i < fieldDtoList.size(); i++) {
			...
			//自分以外
			if (list.getCard_id() != null && !"".equals(list.getCard_id()) && list.getClose() == 0 && list.getField_no() != fieldNumber) {
		...
		if (retTargetList.size() != 0) {
			...
			String targetPlayer = (String)map.get("playerId");

			//対象位置を決める
			ArrayList list = (ArrayList)map.get("list");

			int num2 = rand.nextInt(list.size());
			int target2 = (int)list.get(num2);

			//対象のHPをゼロにする
			BattleFieldDTO fieldDto = fieldDao.getAllValue(battleID, targetPlayer, target2);
			fieldDto.setCur_hp(0);
			fieldDao.setClose(battleID, targetPlayer, target2, fieldDto);
			...
			detailMap.put("fieldNumber", target2);
			detailMap.put("hp", 0);
			retList.add(detailMap);
